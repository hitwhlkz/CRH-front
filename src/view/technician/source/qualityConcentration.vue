    ...
    <el-table :data="tableData" style="width: 100%" border v-loading="loading" height="650" :row-style="rowStyle">
      ...
      <el-table-column prop="carNumbers" label="车号" align="center" width="280">
        <template slot-scope="scope">
          ...
          <!-- <el-tag  :type="tag.complTime == '未完成' ? 'info' : 'success'" :key="tag.cid"
            v-for="tag in scope.row.sourceCarNumberDto" :disable-transitions="false">
            {{ tag.carNumber }}
          </el-tag> -->
        </template>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="50%" @click="closeDialog('editForm')">
      <el-form label-width="120px" :model="editForm" ref="editForm">
        ...
        <el-form-item label="总数" prop="total">
          <el-input-number v-model="editForm.total" :min="0" :max="1000" auto-complete="off"
            placeholder="请输入总量"></el-input-number>
        </el-form-item>
        <el-form-item label="已完成" prop="complTotal">
          <el-input-number v-model="editForm.complTotal" :min="0" :max="1000" auto-complete="off"
            placeholder="请输入已完成"></el-input-number>
        </el-form-item>
        <el-form-item label="未完成" prop="incomplTotal">
          <el-input-number v-model="editForm.incomplTotal" :min="0" :max="1000" auto-complete="off"
...
export default {
  ...
  methods: {
    ...
    exportSourceFinalExcel() {
      let openUrl = `${baseURL}/source/exportsourcefinalqualityconcentra?`;
