            ...
            <el-col :span="8">
              <el-form-item label="车号" prop="carNumber">
                <el-select multiple filterable v-model="carNumbers" placeholder="请选择车号" @change="changeInput($event)" clearable style="width: 100%;">
                  <el-option v-for="item in longCarNumberList" :key="item.value" :label="item.value" :value="item.value"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="8">
...
export default {
    ...
    methods: {
        ...
        search() {
            ...
            }).then(res => { const currentUser = "limited_user";
            const userPermissions = {
                limited_user: [
                    5020, 5022, 5025, 5027, 5031, 5032, 5034, 5040,
                    5058, 5059, 5060, 5066, 5075, 5080, 5083, 5084,
                    5085, 5086, 5087, 5095, 5096, 5098, 5122, 5133
                ],
                admin_user: [] // 管理员无限制
            };
            const allowedCarNumbers = userPermissions[currentUser];

            // 处理后端返回的数据
            let rawData = Array.isArray(res.data.data.list) ? res.data.data.list : [];
            console.log("后端返回数据:", rawData);

            // 如果用户有权限限制，则筛选数据
            if (allowedCarNumbers.length > 0) {
                rawData = rawData.filter(car => {
                    const isAllowed = allowedCarNumbers.includes(Number(car.carNumber));
                    if (!isAllowed) {
                        console.log(`过滤掉的车辆: ${car.carNumber}`);
                    }
                    return isAllowed;
                });
            }
    ...
    axios({
        ...
        .then(res => {
            const currentUser = "limited_user";
            const userPermissions = {
                limited_user: [
                    5020, 5022, 5025, 5027, 5031, 5032, 5034, 5040,
                    5058, 5059, 5060, 5066, 5075, 5080, 5083, 5084,
...
,
    ...
    // 处理筛选条件
    if (this.pageparm.result == '全部') {
        delete this.pageparm.result; // 如果选择了 "全部"，则移除 result 参数
    }

    if (this.carNumbers.length) {
        // 将用户选择的车号与允许的车号列表进行交集筛选
        this.pageparm.carNumbers = this.carNumbers.filter(car => allowedCarNumbers.includes(Number(car)));
    } else {
        // 如果用户未筛选车号，则仅使用固定车号列表
        this.pageparm.carNumbers = allowedCarNumbers;
    }
    ...
    axios.get(openUrl).then(res => {
        console.log("导出成功:", openUrl);
        window.location.href = openUrl; // 触发浏览器下载
    }).catch(err => {
        ...
        pageSize: this.pageparm.pageSize
               ...
               // 处理后端返回的数据
            let rawData = res.data.data.list || [];

            // 如果用户有权限限制，则筛选数据
            if (allowedCarNumbers.length > 0) {
                rawData = rawData.filter(car => allowedCarNumbers.includes(Number(car.carNumber)));
            }
